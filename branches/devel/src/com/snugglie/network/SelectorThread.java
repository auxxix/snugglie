...
public class SelectorThread<T extends MMOClient> extends Thread {
	...
	public SelectorThread(SelectorConfig<T> sc, IMMOExecutor<T> executor,
		...
		DIRECT_WRITE_BUFFER = ByteBuffer
				.allocateDirect(sc.getWriteBufferSize()).order(BYTE_ORDER);
		WRITE_BUFFER = ByteBuffer.wrap(new byte[sc.getWriteBufferSize()])
				.order(BYTE_ORDER);
		READ_BUFFER = ByteBuffer.wrap(new byte[sc.getReadBufferSize()]).order(
				BYTE_ORDER);
	...
	protected void initBufferPool() {
		for (int i = 0; i < HELPER_BUFFER_COUNT; i++) {
			this.getFreeBuffers().addLast(
					ByteBuffer.wrap(new byte[HELPER_BUFFER_SIZE]).order(
							BYTE_ORDER));
		}
	}

	public void openDatagramSocket(InetAddress address, int udpPort)
			throws IOException {
	...
	protected ByteBuffer getPooledBuffer() {
		if (this.getFreeBuffers().isEmpty()) {
			return ByteBuffer.wrap(new byte[HELPER_BUFFER_SIZE]).order(
					BYTE_ORDER);
		} else {
	...
	public SelectionKey registerClientSocket(SelectableChannel sc,
			int interestOps) throws ClosedChannelException {
		...
		for (;;) {
			...
			if (totalKeys > 0) {
				...
				while (iter.hasNext()) {
					key = iter.next();
					iter.remove();
					System.out.println("op" + key.readyOps());
					switch (key.readyOps()) {
			...
			synchronized (this.getPendingClose()) {
				for (n = this.getPendingClose().head(), end = this
						.getPendingClose().tail(); (n = n.getNext()) != end;) {
					con = n.getValue();
		...
		try {
			while ((sc = ((ServerSocketChannel) key.channel()).accept()) != null) {
				if (this.getAcceptFilter() == null
					...
					MMOConnection<T> con = new MMOConnection<T>(this,
							new TCPSocket(sc.socket()), clientKey);
	...
	protected void readTCPPacket(SelectionKey key) {
		...
		if (buf.position() == buf.limit()) {
			// should never happen
			System.err.println("POS ANTES SC.READ(): " + buf.position()
					+ " limit: " + buf.limit());
			System.err.println("NOOBISH ERROR "
					+ (buf == READ_BUFFER ? "READ_BUFFER" : "temp"));
			System.exit(0);
		}

		// System.out.println("POS ANTES SC.READ(): "+buf.position()+" limit: "+
		// buf.limit()+" - buf: "+(buf == READ_BUFFER ? "READ_BUFFER" :
		// "TEMP"));
	...
	protected boolean tryReadPacket2(SelectionKey key, T client, ByteBuffer buf) {
		...
		// System.out.println("BUFF POS ANTES DE LER: "+buf.position()+
		...
		if (buf.hasRemaining()) {
			...
			if (ret != null) {
				...
				if (ret.headerFinished()) {
					...
					// System.out.println("IF: ("+size+" <= "+result+
					// ") => (size <= result)");
					...
					if (size <= result) {
						...
						if (size > 0) {
							...
							this.parseClientPacket(this.getPacketHandler(),
									buf, size, client);
						...
						// we dont have enough bytes for the dataPacket so we
						// need to read
					...
					if (size <= result) {
						if (ret.isMultiPacket()) {
							while (buf.hasRemaining()) {
								this.parseClientPacket(_udpPacketHandler, buf,
										buf.remaining(), client);
							}
						} else {
							// avoid parsing dummy packets (packets without
							// body)
							if (size > 0) {
								int pos = buf.position();
								this.parseClientPacket(_udpPacketHandler, buf,
	...
	protected void parseClientPacket(IPacketHandler<T> handler, ByteBuffer buf,
			int dataSize, T client) {
		int pos = buf.position();

		boolean ret = client.decrypt(buf, dataSize);

		// buf.position(pos); //can be annoying for some decrypt impl decrypt
		// should place the pos at the right place itself

		//System.out.println("pCP -> BUF: POS: "+buf.position()+" - LIMIT: "+buf
		// .limit()+" == Packet: SIZE: "+dataSize);

		if (buf.hasRemaining() && ret) {
			// apply limit
			int limit = buf.limit();
			buf.limit(pos + dataSize);
			//System.out.println("pCP2 -> BUF: POS: "+buf.position()+" - LIMIT: "
			// +buf.limit()+" == Packet: SIZE: "+size);
	...
	@SuppressWarnings("unchecked")
	/*
	 * protected void writePacket(SelectionKey key) { T client = ((T)
	 * key.attachment()); MMOConnection<T> con = client.getConnection();
	 * 
	 * ByteBuffer buf; boolean sharedBuffer = false; if ((buf =
	 * con.getWriteBuffer()) == null) { SendablePacket<T> sp = null;
	 * synchronized (con.getSendQueue()) { if (!con.getSendQueue().isEmpty()) {
	 * sp = con.getSendQueue().removeFirst(); } } if (sp == null) {
	 * System.out.println("OMG WRITE BUT NO WRITE"); return; }
	 * //System.out.println("WRITING: "+sp.getClass().getSimpleName());
	 * this.prepareWriteBuffer(client, sp); buf = sp.getByteBuffer();
	 * //System.out.println("WRITED:: "+sp.getClass().getSimpleName());
	 * //System.out.println("wp:" +buf.position()); buf.flip();
	 * 
	 * sharedBuffer = true; }
	 * 
	 * int size = buf.remaining(); int result = -1;
	 * 
	 * try { result = con.getSocketChannel().write(buf); } catch (IOException e)
	 * { // error handling goes on the if bellow }
	 * 
	 * // check if no error happened if (result > 0) { // check if we writed
	 * everything if (result == size) { //System.out.println("WRITEI COMPLETO");
	 * synchronized (con.getSendQueue()) { if (con.getSendQueue().isEmpty()) {
	 * con.disableWriteInterest(); } }
	 * 
	 * // if it was a pooled buffer we can recycle if (!sharedBuffer) {
	 * this.recycleBuffer(buf); con.setWriteBuffer(null); } } else //incomplete
	 * write { //System.out.println("WRITEI INCOMPLETO"); // if its the main
	 * buffer allocate a pool one if (sharedBuffer) {
	 * con.setWriteBuffer(this.getPooledBuffer().put(buf)); } else {
	 * con.setWriteBuffer(buf); } } } else { con.onForcedDisconnection();
	 * this.closeConnectionImpl(con); } }
	 */
	protected void prepareWriteBuffer(T client, SendablePacket<T> sp) {
		WRITE_BUFFER.clear();
		System.out.println("SendablePacket: " + sp);
	...
	protected void writePacket2(SelectionKey key) {
		...
		try {
			...
			// for (int i = 0; i < DIRECT_WRITE_BUFFER.limit(); i++)
			// {
			// System.out.printf("0x%02x ", DIRECT_WRITE_BUFFER.get(i));
			// }
		...
		if (result >= 0) {
			...
			if (result == size) {
				...
				// if there was a a pending write then we need to finish the
				// operation
				/*
			...
			// incomplete write
			{
				...
				// System.err.println("DEBUG: INCOMPLETE WRITE - write size: "+
				// size);
				// System.err.flush();
			}

			if (result == 0) {
				//System.err.println("DEBUG: write result: 0 - write size: "+size
				// +" - DWB rem: "+DIRECT_WRITE_BUFFER.remaining());
				// System.err.flush();
			}
	...
	protected void prepareWriteBuffer2(MMOConnection<T> con) {
		...
		if (con.hasPendingWriteBuffer()) {
			...
			//System.err.println("ADDED PENDING TO DIRECT "+DIRECT_WRITE_BUFFER.
			// position());
		}
		...
		if (DIRECT_WRITE_BUFFER.remaining() > 1 && !con.hasPendingWriteBuffer()) {
			...
			synchronized (sendQueue) {
				for (n = sendQueue.head(), end = sendQueue.tail(); (n = n
						.getNext()) != end
						&& i++ < MAX_SEND_PER_PASS;) {
					sp = n.getValue();
					...
					if (DIRECT_WRITE_BUFFER.remaining() >= WRITE_BUFFER.limit()) {
						/*
						 * if (i == 0) { // mark begining of new data from
						...
						// con.addWriteBuffer(this.getPooledBuffer().put(
						// WRITE_BUFFER));
	...
	protected final void putPacketIntoWriteBuffer(T client, SendablePacket<T> sp) {
		...
		// System.out.printf("sent      (%d, %d):  ", 0, headerSize + dataSize);
		// Util.printArray(System.out, "(byte) 0x%02x, ", sp.getByteBuffer()
		// .array(), 0, headerSize + dataSize);
	}

	/*
	 * protected void prependHeader(int pos, int size) { switch (HEADER_TYPE) {
	 * case BYTE_HEADER: WRITE_BUFFER.put(pos, (byte) size); break; case
	 * SHORT_HEADER: WRITE_BUFFER.putShort(pos, (short) size); break; case
	 * INT_HEADER: WRITE_BUFFER.putInt(pos, size); break; } }
	 */

	/*
	 * protected int getHeaderValue(ByteBuffer buf) { switch (HEADER_TYPE) {
	 * case BYTE_HEADER: return buf.get() & 0xFF; case SHORT_HEADER: return
	 * buf.getShort() & 0xFFFF; case INT_HEADER: return buf.getInt(); } return
	 * -1; // O.o }
	 */
